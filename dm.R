...
data$nonwhite <- as.numeric(data$Ethnicity2)
data$nonwhite <- ifelse(data$nonwhite == 8, 0, 1)
data$nonwhite[as.numeric(data$Ethnicity2) == 7] <- NA
...
data$sexualmin <- ifelse(data$sexualmin == 1, 0, data$sexualmin)
data$sexualmin[data$sexualmin == 6] <- NA
data$sexualmin <- ifelse(data$sexualmin > 1, 1, data$sexualmin)
...
data$employed3 <- data$D5 - 1
data$employed3[data$D6 == 1] <- 0

data$employed3[data$employed3 == 5] <- 4
data$employed3 <- factor(data$employed3)
...
data$H8[data$H8 == 2] <- 0
data$H9[data$H9 == 2] <- 0
data$H10[data$H10 == 2] <- 0
data$H11[data$H11 == 2] <- 0
...
data <- data %>% 
  mutate(servicesindex = select(., wildcatfoodpantrydummy, basicneedsprojduumy, 
                                offcampusssdummy, studentemergencygrantdummy, 
                                emerghousingdummy, FAadvisordummy) %>% 
           rowSums(na.rm = TRUE))
